var websiteStatistics = new(function() {
    ...
    this.init = function() {
        ...
        ws.data.utm_source = this.statGetQueryString.utm_source ? this.statGetQueryString.utm_source : '';
        ws.data.utm_medium = this.statGetQueryString.utm_medium ? this.statGetQueryString.utm_medium : '';
        ws.data.utm_campaign = this.statGetQueryString.utm_campaign ? this.statGetQueryString.utm_campaign : '';
        ws.data.utm_term = this.statGetQueryString.utm_term ? this.statGetQueryString.utm_term : '';
        ws.data.utm_content = this.statGetQueryString.utm_content ? this.statGetQueryString.utm_content : '';
        var data = ws.getData();
        ...
        ws.data.is_free_user = packageNUM && packageNUM == 1 ? 1 : 0;
        ws.saveStatistics(data, 'save');
    };
    this.trackTimeSpentInit = function() {
        var start = new Date();
        $(document).off('s123.pjax.send.websiteStatistics').on('s123.pjax.send.websiteStatistics', function(event) {
    ...
    this.statGetQueryString = (function(paramsArray) {
        let params = {};
